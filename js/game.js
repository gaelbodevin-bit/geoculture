...
function showPremiumOverlay(featureName) {
  ...
  h.push('<button onclick="window.closeHistory()" style="padding:8px 20px;border-radius:8px;border:1px solid #2d3f5e;background:transparent;color:#6b7280;cursor:pointer;font-size:13px">&#8592; Retour</button>');
...
function redeemCode() {
  ...
  // Sécurité : validation + attribution du premium 100% côté serveur (Cloud Function).
  // Le client n'écrit plus jamais dans testCodes ni users (règles Firestore : write false).
  ...
  redeemFn({ code: code }).then(function(res) {
    ...
    if (/already|utilis/i.test(em)) em = 'Code d\u00e9j\u00e0 utilis\u00e9.';
