...
function showInter(pts,dist,name,eliminated){
  ...
  ov.innerHTML=`
    <div id="${imgId}" style="width:100%;max-width:500px;min-height:120px;background:#111827;border-radius:12px;overflow:visible;position:relative;flex-shrink:0;margin-bottom:clamp(12px,4vh,32px)"></div>
...
function showMenu(){
  ...
  if(user){
    ...
    h.push('<button onclick="window.showHistory&&window.showHistory()" style="font-size:11px;color:#6b7280;background:transparent;border:none;cursor:pointer;text-decoration:underline">Profil</button>');
    h.push('</div>');
  } else {
