// ?? G�oCulture Multijoueur � v3.1 (tous jouent ensemble, panel live, marqueurs)
...
var mpCurrentRound = -1;   // index de la manche actuellement jou�e
var mpAnswered    = false;  // le joueur local a-t-il r�pondu ce round ?
var mpRoundActive = false;  // la manche est-elle en cours (emp�che les r�-init)
...
// ??? �couter le salon (un seul listener sur la room enti�re) ?????????????????
...
function mpUpdateLobby(room) {
  // Ne redessiner que si on est bien en 'waiting' (�vite d'�craser l'�cran de jeu)
  ...
  plist.forEach(function([pid,p]) {
    ...
    if(p.isHost) h.push('<span style="font-size:10px;color:#f97316;background:#3d1a05;padding:2px 6px;border-radius:4px">H�te</span>');
    h.push('</div>');
  });
  h.push('</div>');
  ...
  if(mp.isHost) {
    ...
    h.push('<div style="color:#94a3b8;font-size:13px;padding:10px">En attente que l\'h�te lance...</div>');
  }
...
// L'h�te passe � 'playing' quand c'est termin� � les autres re�oivent la mise � jour Firebase.
var _cdTimer = null;
function mpHandleCountdown(room) {
  ...
  if(remaining > 0) {
    ...
    // L'h�te �crit roundStart dans le futur (+1500ms) pour absorber le d�lai r�seau
    // Tous les clients recevront le m�me timestamp et d�marreront au m�me moment
    if(mp.isHost) {
      var syncedStart = Date.now() + 1500;
      update(mp.roomRef, { status:'playing', roundStart: syncedStart });
    }
    // Les non-h�tes re�oivent status:'playing' via onValue avec le m�me roundStart
  }
}

// ??? D�marrer un round � appel� quand status passe � 'playing' ???????????????
function mpHandlePlaying(room) {
  var rIdx = room.round||0;

  // Si c'est le m�me round et qu'il est d�j� actif ? simple rafra�chissement panel
  ...
  // ?? Panel live � cr�er et forcer visible AVANT que l'overlay soit cach� ??
  mpEnsureLivePanel();
...
function mpStartSyncTimer(roundStart, duration, rIdx) {
  ...
  function tick() {
    var now = Date.now();
    // Attendre que roundStart soit atteint (absorbe le d�lai r�seau c�t� invit�s)
    ...
    if(remaining<=0) {
      clearInterval(mp.timerInterval);
      // Le joueur n'a pas r�pondu ? soumettre rat� automatiquement
...
function mpSubmitAnswer(pos, dist, pts, rIdx) {
  ...
  set(ansRef, {
    ...
    // L'h�te surveille si tout le monde a r�pondu
    if(mp.isHost) mpWatchAllAnswered(rIdx);
  });
}

// ??? H�te surveille les r�ponses de tous ?????????????????????????????????????
    ...
    update(mp.roomRef, { status:'roundEnd', currentRoundResults:rIdx }).then(function() {
      setTimeout(function() {
        update(mp.roomRef, next >= (opts.nbRounds||5)
          ? { status:'finished' }
          : { status:'playing', round:next, roundStart:Date.now()+1500 }); // +1500ms pour absorber d�lai r�seau
      }, 6000);
    });
  });
}
...
function mpHandleRoundEnd(room) {
  ...
  if(window.map && place.lat) {
    ...
    var popup = '<div style="font-family:system-ui,sans-serif;font-size:13px"><b style="color:#15803d">? '+place.name.split('�')[0].trim()+'</b></div>';
  ...
  h.push('<div style="font-size:17px;font-weight:700;color:#e2e8f0;margin-bottom:14px">'+place.name.split('�')[0].trim()+'</div>');
  ...
  h.push('<div style="font-size:12px;color:#6b7280;margin-top:10px">'+(next<nb ? 'Prochaine manche dans 6s�' : 'Fin de partie dans 6s�')+'</div>');
...
function mpUpdateLivePanel(room) {
  // Appel� depuis mpHandleRoomChange (mises � jour temps r�el)
  // Seulement si le panel existe et que l'overlay est cach� (on est en train de jouer)
...
// ??? Hook sur confirmGuess / timer �coul� de game.js ?????????????????????????
// STRAT�GIE : on surcharge window.resolveRound
// En mode MP on bloque l'affichage solo (showInter) et on soumet � Firebase � la place.
// ??? mpOnConfirm : appel� par game.js (confirmGuess/nextLevel) en mode MP ?????
// game.js a �t� patch� pour appeler window.mpOnConfirm() au lieu de resolveRound()
window.mpOnConfirm = function() {
  if(!window._mpMode || !mp.roomCode) return;
  if(mpAnswered) return;
  mpAnswered = true;
  gameActive = false;
  clearInterval(tiv);            // stopper timer solo (s�curit�)
  ...
  if(playerPos && r) {
    ...
    pts  = Math.round(BASE_PTS[level]*0.8*Math.exp(-DIST_K*dist) + (chillMode ? 0 : BASE_PTS[level]*0.2*(timeLeft/30)));
  }
  ...
  if(typeof showToast==='function') {
    showToast(dist!=null ? fmtDst(dist)+' � +'+pts+' pts � En attente des autres...' : 'Rat� � En attente des autres...');
  }
