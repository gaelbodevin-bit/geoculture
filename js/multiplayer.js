...
function mpHandlePlaying(room) {
  ...
  // Si c'est le même round et qu'il est déjà actif ? simple rafraîchissement panel
...
function mpStartSyncTimer(roundStart, duration, rIdx) {
  ...
  function tick() {
    ...
    if(remaining<=0) {
      clearInterval(mp.timerInterval);
      // Le joueur n'a pas répondu ? soumettre raté automatiquement
      if(!mpAnswered) {
        mpAnswered = true;
        gameActive = false;
...
function mpWatchAllAnswered(rIdx) {
  ...
  var _to = setTimeout(function(){ clearInterval(_iv); mpAdvance(rIdx); }, 35000);
...
function mpHookGameEngine() {
  ...
  window.resolveRound = function() {
    ...
    if(mpAnswered) return; // déjà soumis (ex: timeout timer)
    mpAnswered = true;
    gameActive = false;
    clearInterval(tiv);
    clearInterval(mp.timerInterval);
    ...
    if(typeof showToast==='function') showToast(dist!=null ? fmtDst(dist)+' · +'+pts+' pts · En attente des autres...' : 'Raté · En attente des autres...');
