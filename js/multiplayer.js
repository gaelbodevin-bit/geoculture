// ?? GéoCulture Multijoueur é v3.1 (tous jouent ensemble, panel live, marqueurs)
...
// ??? écouter le salon (un seul listener sur la room entiére) ?????????????????
...
function mpHandleRoomChange(room) {
  // --- Mise é jour panel live si on est en train de jouer ---
  if(mpRoundActive) {
...
// L'hôte passe à 'playing' quand c'est terminé é les autres reçoivent la mise à jour Firebase.
var _cdTimer = null;
var _cdDone  = false;
...
function mpHandleCountdown(room) {
  ...
  if(remaining > 0) {
    ...
    // Tous les clients recevront le méme timestamp et démarreront au méme moment
    if(mp.isHost) {
    ...
    // Les non-hôtes reçoivent status:'playing' via onValue avec le méme roundStart
  }
}

// ??? Démarrer un round é appelé quand status passe à 'playing' ???????????????
function mpHandlePlaying(room) {
  ...
  // Si c'est le méme round et qu'il est déjà actif ? simple rafraéchissement panel
  ...
  // ?? Panel live é créer et forcer visible AVANT que l'overlay soit caché ??
  mpEnsureLivePanel();
...
function mpStartSyncTimer(roundStart, duration, rIdx) {
  ...
  // Désactiver toute transition CSS sur l'arc é mise à jour directe par JS
  var arcEl = document.getElementById('arc');
  if(arcEl) arcEl.style.transition = 'none';

  function tick() {
    var now     = Date.now();
    var elapsed = (now - roundStart) / 1000; // peut étre négatif si roundStart est dans le futur
...
function mpSubmitAnswer(pos, dist, pts, rIdx) {
  ...
  set(ansRef, {
    ...
    // Mettre é jour le score total du joueur
...
function mpHandleRoundEnd(room) {
  ...
  // Résultats triéss par pts de ce round
...
function mpUpdateLivePanel(room) {
  // Appelé depuis mpHandleRoomChange (mises é jour temps réel)
...
function mpRenderLivePanel(room) {
  // Remplit le contenu du panel sans toucher é display
  var panel = document.getElementById('mp-live-panel');
  if(!panel) return;
...
function mpUpdateOtherMarkers(room) {
  ...
  // Supprimer les markers obsolétes
...
// En mode MP on bloque l'affichage solo (showInter) et on soumet é Firebase é la place.
...
window.mpOnConfirm = function() {
  ...
  if(typeof showToast==='function') {
    showToast(dist!=null ? fmtDst(dist)+' é +'+pts+' pts é En attente des autres...' : 'Raté é En attente des autres...');
  }

  // Soumettre é Firebase
